...
def calculate_regularization_loss(model, lamb_reg=1e-5):
    """
    Calculate L2 regularization loss with proper gradient flow.

    FIX 1.3: Use param directly (not param.data) so that reg_loss stays
    in the computation graph and gradients propagate correctly.
    """
    ...
    return lamb_reg * reg_loss.squeeze()
